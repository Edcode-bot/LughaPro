...
export function HomeClient() {
  ...
  return (
    <main className="min-h-screen bg-off-white">
      ...
      <section id="tutors" className="mx-auto max-w-7xl px-5 py-20 lg:px-8"><div className="flex flex-col justify-between gap-6 md:flex-row md:items-end"><div><p className="font-bold uppercase tracking-[0.25em] text-gold">Marketplace</p><h2 className="mt-3 font-serif text-4xl font-black text-forest md:text-5xl">Featured Tutors</h2>{miniPay ? <p className="mt-3 inline-flex rounded-full bg-gold px-4 py-2 text-sm font-black text-forest">Pay with MiniPay</p> : null}</div><div className="relative w-full md:max-w-md"><Search className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-forest/45" /><input value={search} onChange={(event) => setSearch(event.target.value)} className="h-13 w-full rounded-full border border-forest/10 bg-white pl-12 pr-5 text-forest outline-none ring-gold/30 transition focus:ring-4" placeholder="Search by name, specialty, price..." /></div></div><div className="mt-8 flex gap-3 overflow-x-auto pb-2 md:flex-wrap">{filters.map((item) => <button key={item.label} onClick={() => setFilter(item.value)} className="shrink-0 py-2"><Badge tone={filter === item.value ? "top" : "default"}>{item.label}</Badge></button>)}</div>{error ? <p className="mt-8 rounded-2xl bg-red-50 p-4 text-red-700">{error}</p> : null}<div className="mt-10 grid gap-6 md:grid-cols-2 xl:grid-cols-3">{loading ? Array.from({ length: 6 }).map((_, index) => <SkeletonCard key={index} />) : tutors.map((tutor) => <TutorCard key={tutor.id} tutor={tutor} />)}</div></section>
