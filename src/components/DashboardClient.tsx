...
export function DashboardClient() {
  ...
  return (
    <main className="min-h-screen bg-off-white pb-24 text-forest lg:pb-0 lg:pl-80">
      <aside className="fixed inset-y-0 left-0 hidden w-80 flex-col border-r border-forest/10 bg-forest p-6 text-cream lg:flex"><div className="flex items-center gap-3"><Avatar name={displayName} online /><div><p className="font-bold">{displayName}</p><p className="text-sm text-cream/60">{address ?? "Wallet not connected"}</p></div></div><nav className="mt-10 grid gap-2">{nav.map(([Icon, label, href], index) => <Link key={label} className={`flex items-center gap-3 rounded-2xl px-4 py-3 font-semibold transition ${index === 0 ? "bg-gold text-forest" : "text-cream/78 hover:bg-white/10"}`} href={href}><Icon className="h-5 w-5" />{label}</Link>)}</nav><button onClick={() => disconnect()} className="mt-auto flex items-center gap-3 rounded-2xl px-4 py-3 font-semibold text-cream/78 hover:bg-white/10"><LogOut className="h-5 w-5" />Disconnect</button></aside>
